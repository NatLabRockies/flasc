...
def _wake_loss(cumprod_waked, cumprod_unwaked):
    ...
    Note that this is not a symmetric metric, and should be interpreted as the percentage loss from
    unwaked to waked production.
    """
...
def _prepare_df_list(df_list: list, exclude_turbs: list, ws_range: list):
    ...
    if not all([all(df_list[0]["time"] == df["time"]) for df in df_list]):
        raise ValueError(
            "Input dataframes have different timestamps. Please ensure all dataframes have the "
            "same timestamps."
        )
    ...
    if len(set(n_turbs_list)) > 1:
        raise ValueError(
            f"Input dataframes have different number of turbines: {n_turbs_list}. Please ensure "
            "all dataframes have the same number of turbines."
        )

    for dfii, df in enumerate(df_list):
        if "pow_ref" in df.columns:
            raise ValueError(
                f"Input dataframe[{dfii}] for {model_tags[dfii]} may not contain 'pow_ref' column. "
                "This may only happen AFTER mirroring NaNs between dataframes and will be done "
                "automatically."
            )
    ...
    N_b = (
        ~df_list[0]["ws"].isna()
    ).sum()  # Number of valid entries in first df before wind speed masking
    ...
    N_a = (
        ~df_list[0]["ws"].isna()
    ).sum()  # Number of valid entries in first df after wind speed masking
    if N_a < N_b:
        print(
            f"Masking down to wind speed range {ws_range[0]:.1f} m/s to {ws_range[1]:.1f} m/s. "
            + f"Number of measurements before: {N_b}, after: {N_a}. Percentage: "
            + f"{100.0 * N_a / N_b:.1f}%."
        )
    ...
    timespan = t1 - t0
    n_measurements_per_hour = np.timedelta64(1, "h") / (
        df_list[0].iloc[1]["time"] - df_list[0].iloc[0]["time"]
    )
...
def compare_cumulative_production(
    ...
    cumprod_turbine_list = [
        [(df[pc].sum() * 1.0e-6 / n_measurements_per_hour) for pc in pow_cols] for df in df_list
    ]
    ...
    for mii, model_tag in enumerate(model_tags):
        table_absolute_cumprod_dict[f"{model_tag} (MWh)"] = [
            cumprod_farm_list[mii]
        ] + cumprod_turbine_list[mii]
...
def compare_relative_wake_loss(
    ...
    cumprod_turbine_waked_list = [
        np.array([None for _ in range(n_turbs)], dtype=float) for _ in df_list
    ]
    cumprod_turbine_unwaked_list = [
        np.array([None for _ in range(n_turbs)], dtype=float) for _ in df_list
    ]
    ...
    for ti in range(n_turbs):
        for dii, df in enumerate(df_list):
            ...
            cumprod_turbine_unwaked_list[dii][ti] = (
                np.sum(p_ref[ids_non_nan]) * 1.0e-6 / n_measurements_per_hour
            )
            cumprod_turbine_waked_list[dii][ti] = (
                np.sum(p_test[ids_non_nan]) * 1.0e-6 / n_measurements_per_hour
            )

    cumprod_farm_waked_list = [
        np.sum(cumprod_tm_waked) for cumprod_tm_waked in cumprod_turbine_waked_list
    ]
    cumprod_farm_unwaked_list = [
        np.sum(cumprod_tm_unwaked) for cumprod_tm_unwaked in cumprod_turbine_unwaked_list
    ]
    ...
    for mii, model_tag in enumerate(model_tags):
        ...
        if mii > 0:  # Compare against first entry (usually SCADA or LES) and calculate error
            table_wakeloss_cumprod_dict[f"{model_tag} error (p.p.)"] = (
                wake_losses_list[mii] - wake_losses_list[0]
            )
